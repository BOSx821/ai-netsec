...
import { SocAlertCard } from "@/components/SocAlertCard";
  ...
  return (
    <div className="space-y-7">
      ...
      <section className="space-y-3 md:hidden">
        ...
        {alertsQuery.data?.map(({ alert, assetName }) => <SocAlertCard key={alert.id} alert={alert} assetName={assetName} onDetails={() => setSelectedAlertId(alert.id)} disabled={updateStatus.isPending} onStatusChange={status => updateStatus.mutate({ id: alert.id, status })} />)}
      </section>
