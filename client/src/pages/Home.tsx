...
function DashboardContent() {
  ...
  return <div className="space-y-6">
    <SocPageHeader eyebrow="Vue opérationnelle" title="Posture de sécurité" description="Évaluez le risque actuel, priorisez les signaux et accédez directement aux actions d’investigation." actions={<>{user?.role === "admin" && !dashboardQuery.data?.isDemoData ? <Button size="sm" variant="outline" className="soc-secondary-button" disabled={seedDemo.isPending} onClick={() => seedDemo.mutate()}><DatabaseZap className="mr-2 size-3.5" />Charger les données de démo</Button> : null}</>} />
    ...
    <section className={`soc-threat-brief soc-threat-brief--${risk.tone}`} aria-labelledby="threat-brief-title"><div className="soc-threat-icon"><ShieldAlert className="size-6" /></div><div className="min-w-0 flex-1"><p className="soc-eyebrow">État prioritaire</p><div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1"><h2 id="threat-brief-title">{risk.label}</h2>{metrics ? <span className="font-mono text-sm text-slate-300">{metrics.globalRiskScore}/100</span> : null}</div><p>{risk.description}</p></div><div className="flex shrink-0 flex-wrap items-center gap-2"><span className="soc-count-chip"><BellRing className="size-3.5" aria-hidden="true" />{criticalAlerts} alerte{criticalAlerts > 1 ? "s" : ""} critique{criticalAlerts > 1 ? "s" : ""}</span><Button size="sm" onClick={() => setLocation("/alerts")}>Voir les alertes <ArrowUpRight className="ml-2 size-3.5" /></Button></div></section>

    <section className="grid gap-4 sm:grid-cols-2 2xl:grid-cols-4" aria-label="Indicateurs clés de sécurité"><SecurityKpiCard icon={BellRing} label="Alertes actives" value={metrics?.activeAlerts ?? "—"} detail={`${criticalAlerts} critique${criticalAlerts > 1 ? "s" : ""} à examiner`} tone={criticalAlerts > 0 ? "critical" : "info"} /><SecurityKpiCard icon={Activity} label="Incidents ouverts" value={metrics?.openIncidents ?? "—"} detail="Dossiers nécessitant un suivi" tone={(metrics?.openIncidents ?? 0) > 0 ? "high" : "success"} /><SecurityKpiCard icon={Gauge} label="Score de risque" value={metrics ? `${metrics.globalRiskScore}/100` : "—"} detail={risk.shortLabel} tone="info" /><SecurityKpiCard icon={Network} label="Couverture systèmes" value={systemsStatus} detail="Disponibles / inventoriés" tone={metrics && metrics.onlineSystems < metrics.monitoredSystems ? "high" : "success"} /></section>
